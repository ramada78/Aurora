  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-3 sm:mb-4 pb-2 border-b border-gray-200">
        <h2 className={`text-lg sm:text-xl font-semibold text-gray-800 flex items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
          <Info className={`w-4 h-4 sm:w-5 sm:h-5 ${isRTL ? 'ml-1.5' : 'mr-1.5'} sm:${isRTL ? 'ml-2' : 'mr-2'} flex-shrink-0 text-blue-600`} />
          <span className="truncate">{t('aiAgent.analysis.title')}</span>
        </h2>
        <button 
          onClick={() => setExpanded(!expanded)} 
          className="text-gray-500 hover:text-blue-600 transition-colors p-1 rounded-full hover:bg-gray-100 flex-shrink-0"
          aria-label={expanded ? t('aiAgent.analysis.collapse') : t('aiAgent.analysis.expand')}
        >
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            ...
            <div className={`mt-5 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100 flex items-center text-xs text-gray-500 italic ${isRTL ? 'flex-row-reverse' : ''}`}>
              <Info className={`w-3 h-3 ${isRTL ? 'ml-1.5' : 'mr-1.5'} flex-shrink-0`} />
              <span>{t('aiAgent.analysis.disclaimer')}</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
